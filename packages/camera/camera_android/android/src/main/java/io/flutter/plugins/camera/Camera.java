...
class Camera
    ...
    public static class VideoCaptureSettings {
        ...
        public VideoCaptureSettings(@NonNull ResolutionPreset resolutionPreset, boolean enableAudio,
                                    boolean enableStabilization) {
            this(resolutionPreset, enableAudio, enableStabilization, null, null, null);
        }
    }
    ...
    private void createCaptureSession(
        ...
        if (videoCaptureSettings.enableStabilization) {
            Log.i(TAG, "Enabling stabilization");

            // Determining the optical stabilization mode
            int[] availableOpticalStabilization = characteristics
                    .get(CameraCharacteristics.LENS_INFO_AVAILABLE_OPTICAL_STABILIZATION);
            int opticalStabilizationMode = CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE_OFF;
            if (availableOpticalStabilization != null) {
                for (int mode : availableOpticalStabilization) {
                    if (mode == CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE_ON) {
                        opticalStabilizationMode = mode;
                    }
                }
            }

            // Determining the digital stabilization mode
            int[] availableVideoStabilization = characteristics
                    .get(CameraCharacteristics.CONTROL_AVAILABLE_VIDEO_STABILIZATION_MODES);
            int videoStabilizationMode = CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE_OFF;
            if (availableVideoStabilization != null) {
                for (int mode : availableVideoStabilization) {
                    if (mode == CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE_ON && mode > videoStabilizationMode) {
            ...
            // Apply the stabilization mode to the previewRequestBuilder
            //
            // Making sure that both of them are never active at the same time
            // as stated in the Camera2-API documentation.
            if (opticalStabilizationMode != CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE_OFF) {
                ...
                Log.i(TAG, "Optical stabilization mode: " + opticalStabilizationMode);
            } else if (videoStabilizationMode != CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE_OFF) {
                previewRequestBuilder.set(CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE,
                        CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE_OFF);
                previewRequestBuilder.set(CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE, videoStabilizationMode);
                Log.i(TAG, "Digital stabilization mode: " + videoStabilizationMode);
            } else {
                previewRequestBuilder.set(CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE,
                        CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE_OFF);
                previewRequestBuilder.set(CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE,
                        CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE_OFF);
                Log.i(TAG, "Disabled stabilization");
            }
        } else {
            previewRequestBuilder.set(CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE,
                    CaptureRequest.LENS_OPTICAL_STABILIZATION_MODE_OFF);
